...
optional arguments and value type, literal/range/symbol rule items, rule
actions, and surrounding source template text. Keep semantic helpers in this
file close to the grammar rules so a new parser snapshot can be regenerated
...
namespace mparse::spec::generated {
    ...
    std::vector<std::string> splitCommaSeparatedExpressions(std::string_view content) {
        content = mparse::spec::trim(content);
...
static std::any mparse_action_77(const std::vector<std::any>& args) {
    ...
    resolved_args.push_back(args.at(0));
    {
        ...
        resolved_args.push_back(mparse_action_78(nested_args));
    }
